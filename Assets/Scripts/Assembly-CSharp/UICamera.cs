...
public class UICamera : MonoBehaviour
{
	...
	static UICamera()
	{
		...
		GetKeyDown = (KeyCode key) => (key < KeyCode.JoystickButton0 || !ignoreControllerInput) && Input.GetKeyDown(key);
		GetKeyUp = (KeyCode key) => (key < KeyCode.JoystickButton0 || !ignoreControllerInput) && Input.GetKeyUp(key);
		GetKey = (KeyCode key) => (key < KeyCode.JoystickButton0 || !ignoreControllerInput) && Input.GetKey(key);
		GetAxis = (string axis) => ignoreControllerInput ? 0f : Input.GetAxis(axis);
		GetMouse = (int button) => mMouse[button];
